...
class ProxyListener implements \wcf\system\event\listener\IParameterizedEventListener {
	...
	public function execute($eventObj, $className, $eventName, array &$parameters) {
		...
		// match all [img=link,(left|right|center),[0-9]+]
		preg_match_all("~\[img=\'([^\]]*)\',(left|right|center),([0-9]+)\]\[\/img\]~i", $eventObj->message, $matches2, PREG_SET_ORDER);
		
		// match all [img=link,(left|right|center)]
		preg_match_all("~\[img=\'([^\]]*)\',(left|right|center)\]\[\/img\]~i", $eventObj->message, $matches3, PREG_SET_ORDER);

		// match [img=link]
		preg_match_all("~\[img=\'?([^\,\]]*)\'?\]~i", $eventObj->message, $matches4, PREG_SET_ORDER);
		...
		foreach ($matches as $match) {
			if (function_exists('gethostbyname')) {
				...
				if (!$localhost && !\wcf\system\application\ApplicationHandler::getInstance()->isInternalURL($match[1])) {
					$eventObj->message = \wcf\util\StringUtil::replaceIgnoreCase($match[0], '[img=\''. $this->buildImageURL($match[1]) .'\''. ((isset($match[2])) ? ','.$match[2].((isset($match[3])) ? ','.$match[3] : '') : '') .'][/img]', $eventObj->message);
				}
			}
		}
	}
